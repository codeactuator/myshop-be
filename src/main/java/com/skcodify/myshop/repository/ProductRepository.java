...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query("SELECT p FROM Product p JOIN User u ON p.userId = u.id WHERE u.society.id = :societyId")
    List<Product> findBySocietyId(@Param("societyId") Long societyId);

    @Query("SELECT p FROM Product p JOIN User u ON p.userId = u.id WHERE p.status = :status AND u.society.id = :societyId")
